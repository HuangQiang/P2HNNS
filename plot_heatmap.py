...
def plot_heatmap(m, interval, datasets, input_folder, output_folder, 
    ...
    for dataset in datasets:
        file_name = input_folder + dataset + '_' + str(m) + '_Heatmap.out'
    ...
    for i, (grid, data_label) in enumerate(zip(grids, dataset_labels)):
        ...
        ax0.set_title(data_label, loc='center', y=1.1, x=(hist_margin+marginal_width)/2+0.1, fontsize=18)
        ...
        plt.yticks(m_ticks)
        ax1.set_xlabel(r'Euclidean Norm $\Vert \mathbf{o} \Vert$ (%)', fontsize=16)
        if (data_label == "Yelp"): 
    ...
    # plt.tight_layout()
    plt.savefig('%sheatmap_%d.pdf' % (output_folder, m))
    plt.savefig('%sheatmap_%d.png' % (output_folder, m))
    plt.savefig('%sheatmap_%d.eps' % (output_folder, m))
    # plt.show()
