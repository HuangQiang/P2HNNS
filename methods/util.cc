    ...
    float *data)                        // data (return)
{
    ...
    for (int i = 0; i < n*d; i += d) {
        fread(&data[i], sizeof(float), d-1, fp);
        data[i+d-1] = 1.0f;
    }
    fclose(fp);
    ...
    float *query)                       // query (return)
{
    ...
    fread(query, sizeof(float), qn*d, fp);
    fclose(fp);
    gettimeofday(&g_end_time, NULL);
    ...
    const float *data)                  // data objects
{
    ...
    char output_set[200];
    sprintf(output_set, "%s%s_%d_L2_Norm_Distr.out", output_folder, 
        data_name, m);
    ...
    const float *query)                 // query objects
{
    ...
    char output_set[200];
    sprintf(output_set, "%s%s_%d_Cos_Angle_Distr.out", output_folder, 
        data_name, m);
